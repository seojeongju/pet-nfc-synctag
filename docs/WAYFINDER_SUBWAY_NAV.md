...
| 메인 | 보조 |
|------|------|
| `/wayfinder` — GPS·근처 역·이동 경로(카카오맵) | `/wayfinder/s/{slug}` — NFC·QR 지점 스팟 안내 |
| 대시보드: 방문자 안내 링크·역 중심 카피 | 대시보드: 스팟 등록·발행·인벤토리 연동 |

**두 갈래 진입**

- **GPS** → 근처 역 → **1~4단계** (역·시설·공공데이터) — **메인**
- **NFC·QR** → 발행된 스팟 URL → **0단계** (지점 고정 안내) — **보조**
