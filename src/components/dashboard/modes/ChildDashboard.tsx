...
export default function ChildDashboard({
  ...
  const handleQuickNfcRegister = () => {
    ...
    startTransition(async () => {
      try {
        ...
        setTagMessage({ type: "success", text: "NFC 태그가 아이 프로필에 연결되었습니다." });
        setTagId("");
        router.refresh();
      } catch (e: unknown) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-outfit pb-32 overflow-x-hidden relative">
      <div className="absolute top-0 left-0 w-full h-[300px] bg-gradient-to-b from-sky-500/10 to-transparent pointer-events-none" />
      ...
      >
        ...
        <motion.section variants={itemVariants}>
          <Card className="rounded-[32px] border-none shadow-app bg-white">
            <CardContent className="p-6 space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-2xl bg-sky-50 text-sky-500 flex items-center justify-center">
                  <Smartphone className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-black text-slate-900">NFC 빠른 등록</h3>
                  <p className="text-[11px] text-slate-400 font-bold">인식표나 가방 태그를 폰에 대거나 UID를 입력하여 연결하세요.</p>
                </div>
              </div>
              ...
              {subjects.length > 0 ? (
                <>
                  <select
                    ...
                    className="w-full h-12 rounded-2xl border border-slate-100 bg-slate-50 px-4 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-sky-500/20"
                  >
                    {subjects.map((s) => (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      className="h-12 rounded-2xl bg-slate-900 hover:bg-sky-500 text-white px-5 font-black"
                    >
                      등록
                    </Button>
                  </div>
                ...
                <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-center space-y-2">
                  <p className="text-xs font-bold text-slate-500">먼저 아이 프로필을 등록해야 태그를 연결할 수 있어요.</p>
                  <a
                    ...
                    className={cn(
                      ...
                      tenantSuspended ? "pointer-events-none text-slate-400" : "text-sky-600"
                    )}
                  >
                    등록하러 가기
                  </a>
              ...
              {tagMessage && (
                <div
                  className={`rounded-2xl px-4 py-3 text-xs font-bold flex items-center gap-2 ${
                    tagMessage.type === "success" ? "bg-sky-50 text-sky-600" : "bg-rose-50 text-rose-500"
                  }`}
                >
        ...
        <motion.section variants={itemVariants} className="space-y-4">
           <div className="flex items-center justify-between px-2">
              ...
              <a href={`/dashboard/pets${kindQs}`} className="text-[10px] font-black text-sky-600 uppercase tracking-widest hover:underline transition-all">View All</a>
           </div>
