...
export default function GoldDashboard({
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-outfit pb-32 overflow-x-hidden relative">
      <div className="absolute top-0 left-0 w-full h-[300px] bg-gradient-to-b from-amber-500/10 to-transparent pointer-events-none" />
      ...
      >
            ...
            <div className="glass-dark rounded-[32px] p-5 text-white flex items-center justify-between shadow-2xl">
              ...
              <a
                ...
                className={cn(
                  ...
                  "inline-flex items-center justify-center bg-amber-500 hover:bg-amber-600 text-white font-black rounded-xl text-[10px] h-8 px-4 transition-all active:scale-90"
                )}
              >
                관리센터 이동
              </a>
        ...
        <motion.section variants={itemVariants} className="flex items-center justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-0.5 text-[10px] font-black text-amber-700">
                골드·인증서 모드
              </span>
              <a href="/hub" className="text-[10px] font-black text-slate-400 hover:text-amber-600 uppercase tracking-widest">
                모드 변경
              </a>
            </div>
            {tenantId && (
            ...
            <h1 className="text-2xl font-black text-slate-900 leading-tight">
               안녕하세요, <br />
               <span className="text-amber-500">{session.user.name || "사용자"}</span>님! 👋
            </h1>
          </div>
          <div className="relative group">
            <div className="absolute inset-0 bg-amber-200 rounded-full blur-md opacity-0 group-hover:opacity-40 transition-opacity" />
            <div className="w-14 h-14 rounded-full border-4 border-white shadow-xl overflow-hidden relative z-10 bg-white">
               {session.user.image ? (
                 ...
                 <div className="w-full h-full flex items-center justify-center bg-amber-50 text-amber-500"><AvatarIcon className="w-6 h-6" /></div>
               )}
            </div>
          </div>
        </motion.section>
        ...
        <motion.section variants={itemVariants} className="relative group">
          ...
          <input
            type="text"
            placeholder="제품 이름을 찾아보세요..."
            className="w-full h-16 glass rounded-[24px] pl-14 pr-5 text-sm font-bold shadow-app shadow-app-hover outline-none transition-all focus:ring-2 focus:ring-amber-500/20"
          />
        </motion.section>
            ...
            <div className="p-8 space-y-5 relative z-10">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[10px] font-black tracking-widest text-amber-400 uppercase">인증·안심</span>
                </div>
                <h2 className="text-xl font-black leading-[1.2]">골드·인증 동행</h2>
              </div>
        ...
        <motion.section variants={itemVariants} className="space-y-4">
           <div className="flex items-center justify-between px-2">
              <h3 className="text-lg font-black text-slate-900">등록한 골드·주얼리</h3>
              <a href={`/dashboard/${subjectKind}/pets${kindQs}`} className="text-[10px] font-black text-amber-600 uppercase tracking-widest hover:underline transition-all">View All</a>
           </div>
           ...
           {items.some((i) => i.is_lost) && (
             <div className="flex items-start gap-3 rounded-[20px] bg-amber-500 px-4 py-3 shadow-lg shadow-amber-300/30">
               ...
               <div className="flex-1 min-w-0">
                 <p className="text-xs font-black text-white">🚨 분실/위험 신고 중인 제품이 있어요</p>
                 <p className="text-[10px] text-white/80 font-bold mt-0.5">
                   {items.filter((i) => i.is_lost).map((i) => i.name).join(", ")} — 제품 분실 모드 활성화 중입니다.
                 </p>
               </div>
             </div>
           )}
           ...
           <div className="flex gap-4 overflow-x-auto pb-6 pt-2 scrollbar-hide -mx-5 px-5">
                  ...
                  <a href={`/profile/${i.id}${kindQs}`}>
                    <Card className="rounded-[32px] border-none shadow-app shadow-app-hover overflow-hidden bg-white text-center p-0">
                       ...
                       <CardContent className="p-4 space-y-0.5">
                          ...
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{i.breed || "GOLD"}</p>
                       </CardContent>
                    </Card>
                  </a>
                </motion.div>
              ...
              <motion.div whileTap={{ scale: tenantSuspended ? 1 : 0.95 }}>
                ...
                >
                   <div className="min-w-[150px] h-full min-h-[176px] rounded-[32px] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-2 hover:bg-amber-50/50 hover:border-amber-500 transition-all group">
                      <div className="w-10 h-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-amber-500 group-hover:text-white transition-all shadow-sm">
                         <Plus className="w-5 h-5" />
                      </div>
                      <span className="text-[10px] font-black text-slate-400 group-hover:text-amber-500 uppercase tracking-wider">추가</span>
                   </div>
                </a>
              </motion.div>
           </div>
        ...
        <motion.section variants={itemVariants} className="space-y-4">
           <div className="flex items-center justify-between px-2">
              <h3 className="text-lg font-black text-slate-900">최근 보호·인증 활동</h3>
           ...
           <Card className="rounded-[32px] border-none shadow-app p-6 bg-white relative overflow-hidden group hover:bg-slate-50 transition-colors">
              <div className="flex items-center gap-4 relative z-10">
                 <div className="w-14 h-14 rounded-[20px] bg-amber-50 flex items-center justify-center text-amber-500 shadow-sm shadow-amber-100">
                    <Bell className="w-6 h-6 animate-pulse" />
                 </div>
                 <div className="space-y-0.5">
                    <h4 className="font-black text-slate-800 text-sm">최근 스캔 내역이 없습니다</h4>
                    <p className="text-[10px] text-slate-400 font-bold leading-relaxed">모든 제품의 가치가 안전하게 증명되고 있습니다.</p>
                 </div>
              </div>
              <div className="absolute bottom-[-20%] right-[-10%] w-32 h-32 bg-slate-50 rounded-full group-hover:bg-amber-50 transition-colors" />
           </Card>
        </motion.section>
      </motion.div>
    </div>
