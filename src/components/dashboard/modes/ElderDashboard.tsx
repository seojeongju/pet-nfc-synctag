...
import {
  Plus, MapPin, UserRound, Search, Bell,
...
interface ElderDashboardProps {
  ...
  subjects: Array<{ id: string; name: string; breed?: string | null; photo_url?: string | null; is_lost?: number | null }>;
  isAdmin: boolean;
...
export default function ElderDashboard({
  ...
  const handleQuickNfcRegister = () => {
    ...
    startTransition(async () => {
      try {
        await linkTag(selectedSubjectId, tagId.trim());
        setTagMessage({ type: "success", text: "NFC 태그가 가족 프로필에 연결되었습니다." });
        setTagId("");
        router.refresh();
      } catch (e: unknown) {
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-outfit pb-32 overflow-x-hidden relative">
      ...
      >
            ...
            <div className="glass-dark rounded-[32px] p-5 text-white flex items-center justify-between shadow-2xl">
              ...
              <a
                ...
                className={cn(
                  ...
                  "inline-flex items-center justify-center bg-indigo-500 hover:bg-indigo-600 text-white font-black rounded-xl text-[10px] h-8 px-4 transition-all active:scale-90"
                )}
              >
                관리센터 이동
              </a>
        ...
        <motion.section variants={itemVariants} className="flex items-center justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="inline-flex items-center rounded-full bg-indigo-50 px-2.5 py-0.5 text-[10px] font-black text-indigo-700">
                기억 동행 모드
              </span>
              <a href="/hub" className="text-[10px] font-black text-slate-400 hover:text-indigo-600 uppercase tracking-widest">
                모드 변경
              </a>
            </div>
            {tenantId && (
              <div className="inline-flex flex-col gap-1 rounded-xl border border-indigo-100 bg-indigo-50 px-3 py-2">
                <p className="text-[10px] font-black text-indigo-700">{tenantUsage?.tenantName}</p>
                {tenantUsage ? (
            ...
            <div className="flex items-center gap-1.5 text-indigo-600 font-bold text-[11px] uppercase tracking-wider">
               <MapPin className="w-3.5 h-3.5" />
               SEOUL, KOREA
            </div>
            <h1 className="text-2xl font-black text-slate-900 leading-tight">
               안녕하세요, <br />
               <span className="text-indigo-500">{session.user.name || "보호자"}</span>님! 👋
            </h1>
          </div>
          <div className="relative group">
            <div className="absolute inset-0 bg-indigo-200 rounded-full blur-md opacity-0 group-hover:opacity-40 transition-opacity" />
            <div className="w-14 h-14 rounded-full border-4 border-white shadow-xl overflow-hidden relative z-10 bg-white">
               {session.user.image ? (
                 ...
                 <div className="w-full h-full flex items-center justify-center bg-indigo-50 text-indigo-500"><AvatarIcon className="w-6 h-6" /></div>
               )}
            </div>
          </div>
        </motion.section>
        ...
        <motion.section variants={itemVariants} className="relative group">
          ...
          <input
            type="text"
            placeholder="가족 이름을 찾아보세요..."
            className="w-full h-16 glass rounded-[24px] pl-14 pr-5 text-sm font-bold shadow-app shadow-app-hover outline-none transition-all focus:ring-2 focus:ring-indigo-500/20"
          />
        </motion.section>
            ...
            <div className="p-8 space-y-5 relative z-10">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[10px] font-black tracking-widest text-indigo-400 uppercase">기억 동행</span>
                </div>
                <h2 className="text-xl font-black leading-[1.2]">가족 안심 동행</h2>
              </div>
            ...
            <CardContent className="p-6 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 font-bold">인식표의 태그를 폰에 갖다 대거나 UID를 입력하여 연결하세요.</p>
                </div>
              </div>
              ...
              {subjects.length > 0 ? (
                <>
                  <select
                    ...
                    className="w-full h-12 rounded-2xl border border-slate-100 bg-slate-50 px-4 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500/20"
                  >
                    {subjects.map((s) => (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      disabled={tenantSuspended || isPending || !selectedSubjectId || !tagId.trim()}
                      className="h-12 rounded-2xl bg-slate-900 hover:bg-indigo-500 text-white px-5 font-black"
                    >
                      등록
                    </Button>
                  </div>
                ...
                <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-center space-y-2">
                  ...
                  <a
                    ...
                    className={cn(
                      ...
                      tenantSuspended ? "pointer-events-none text-slate-400" : "text-indigo-600"
                    )}
                  >
                    등록하러 가기
                  </a>
              ...
              {tagMessage && (
                <div
                  className={`rounded-2xl px-4 py-3 text-xs font-bold flex items-center gap-2 ${
                    tagMessage.type === "success" ? "bg-indigo-50 text-indigo-600" : "bg-rose-50 text-rose-500"
                  }`}
                >
        ...
        <motion.section variants={itemVariants} className="space-y-4">
           ...
           {subjects.some((s) => s.is_lost) && (
             <div className="flex items-start gap-3 rounded-[20px] bg-amber-500 px-4 py-3 shadow-lg shadow-amber-300/30">
               <AlertTriangle className="w-5 h-5 text-white shrink-0 mt-0.5 animate-pulse" />
               <div className="flex-1 min-w-0">
                 <p className="text-xs font-black text-white">🚨 보호가 필요한 가족이 있어요</p>
                 <p className="text-[10px] text-white/80 font-bold mt-0.5">
                   {subjects.filter((s) => s.is_lost).map((s) => s.name).join(", ")} — 실종/보호 활성화 중입니다.
                 </p>
               </div>
             </div>
           )}
           ...
           <div className="flex gap-4 overflow-x-auto pb-6 pt-2 scrollbar-hide -mx-5 px-5">
                  ...
                  <a href={`/profile/${s.id}${kindQs}`}>
                    <Card className="rounded-[32px] border-none shadow-app shadow-app-hover overflow-hidden bg-white text-center p-0">
                       ...
                       <CardContent className="p-4 space-y-0.5">
                          ...
                          <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">{s.breed || "FAMILY"}</p>
                       </CardContent>
                    </Card>
                  </a>
                </motion.div>
              ...
              <motion.div whileTap={{ scale: tenantSuspended ? 1 : 0.95 }}>
                ...
                >
                   <div className="min-w-[150px] h-full min-h-[176px] rounded-[32px] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center gap-2 hover:bg-indigo-50/50 hover:border-indigo-500 transition-all group">
                      <div className="w-10 h-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-indigo-500 group-hover:text-white transition-all shadow-sm">
                         <Plus className="w-5 h-5" />
                      </div>
                      <span className="text-[10px] font-black text-slate-400 group-hover:text-indigo-500 uppercase tracking-wider">추가</span>
                   </div>
                </a>
              </motion.div>
           </div>
        ...
        <motion.section variants={itemVariants} className="space-y-4">
           ...
           <Card className="rounded-[32px] border-none shadow-app p-6 bg-white relative overflow-hidden group hover:bg-slate-50 transition-colors">
              <div className="flex items-center gap-4 relative z-10">
                 <div className="w-14 h-14 rounded-[20px] bg-indigo-50 flex items-center justify-center text-indigo-500 shadow-sm shadow-indigo-100">
                    <Bell className="w-6 h-6 animate-pulse" />
                 </div>
                 <div className="space-y-0.5">
                    <h4 className="font-black text-slate-800 text-sm">최근 스캔 내역이 없습니다</h4>
                    <p className="text-[10px] text-slate-400 font-bold leading-relaxed">모두 안전하게 보호되고 있습니다.</p>
                 </div>
              </div>
              <div className="absolute bottom-[-20%] right-[-10%] w-32 h-32 bg-slate-50 rounded-full group-hover:bg-indigo-50 transition-colors" />
           </Card>
        </motion.section>
      </motion.div>
    </div>
