      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className={adminUi.pageContainer}>
        <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 text-xs sm:text-sm font-bold">시스템 전체 NFC 마스터 데이터 관리 및 인벤토리 추적</p>
          </div>

          <Link
            href="/admin"
            prefetch={false}
            className="flex items-center gap-2 text-xs font-black text-slate-500 hover:text-slate-900 transition-colors group self-start md:self-auto"
          >
